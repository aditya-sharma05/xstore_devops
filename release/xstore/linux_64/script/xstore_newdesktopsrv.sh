...
# Application
APP_NAME="xstore_newdesktopsrv"
APP_LONG_NAME="Xstore New Desktop Server"
...
# description: Xstore New Desktop Server
...
WRAPPER_CONF=$REALPATH/wrapper/conf/xstore_newdesktopsrv.conf
APPLICATION_DIRECTORY="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export PLATFORM=linux_64
export _WRAPPED_APP=xstore_newdesktopsrv
export JAVA_HOME=$(cd "${APPLICATION_DIRECTORY}/../jre" && pwd)
export JAVA=$JAVA_HOME/bin/xstore_newdesktopsrv
PLATFORMPATH=$REALPATH/$PLATFORM
OS_ADDITIONAL_PROP_1=wrapper.java.additional.96=-Dlog.dir.name=../xstoredata/xstore/log/xstore_newdesktopsrv
...
WRAPPER_CMD=$PLATFORMPATH/bin/xstore_newdesktopsrv-wrapper
...
console() {
    ...
    getpid
    if [ "X$pid" = "X" ] ; then
        COMMAND_LINE="$CMDNICE $WRAPPER_CMD $WRAPPER_CONF $OS_ADDITIONAL_PROP_1 wrapper.java.command=$JAVA wrapper.syslog.ident=$APP_NAME wrapper.pidfile=$PIDFILE $ANCHORPROP $LOCKPROP $*"
        exec $COMMAND_LINE
    else
