...
# Application
APP_NAME="xstore_services"
APP_LONG_NAME="Xstore REST Services"
...
# description: Xstore REST Services
...
WRAPPER_CONF=$REALPATH/wrapper/conf/xstore_services.conf
APPLICATION_DIRECTORY="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export PLATFORM=linux_64
export _WRAPPED_APP=xstore_services
export JAVA_HOME=$(cd "${APPLICATION_DIRECTORY}/../jre" && pwd)
export JAVA=$JAVA_HOME/bin/xstore_services
PLATFORMPATH=$REALPATH/$PLATFORM
OS_ADDITIONAL_PROP_1=wrapper.java.additional.96=-Dlog.dir.name=../xstoredata/xstore/log/xstore_services
...
WRAPPER_CMD=$PLATFORMPATH/bin/xstore_services-wrapper
