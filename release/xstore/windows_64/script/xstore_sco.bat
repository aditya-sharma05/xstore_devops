...
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\xstore_sco.conf"
set _WRAPPED_APP=xstore_sco
set PLATFORM=windows_64
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=%ROOT_DIR%jre
set JAVA=%JAVA_HOME%\bin\xstore_sco.exe
set PIDDIR=%_REALPATH%tmp\
set OS_ADDITIONAL_PROP_1=wrapper.java.additional.96=-Dlog.dir.name=../xstoredata/xstore/log/xstore_sco
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\xstore_sco-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Self Checkout POS once installed as a Windows service
    echo     %0 restart    restart Xstore Self Checkout POS once installed as a Windows service
    echo     %0 stop       stop Xstore Self Checkout POS once installed as a Windows service
    echo     %0 install    install Xstore Self Checkout POS as a Windows service
    echo     %0 remove     uninstall Xstore Self Checkout POS as a Windows service
...
:console
SET my_title=Xstore Self Checkout POS Console (%RANDOM%)
