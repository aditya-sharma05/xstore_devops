...
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\xstore_services.conf"
set _WRAPPED_APP=xstore_services
set PLATFORM=windows_64
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=%ROOT_DIR%jre
set JAVA=%JAVA_HOME%\bin\xstore_services.exe
set PIDDIR=%_REALPATH%tmp\
set OS_ADDITIONAL_PROP_1=wrapper.java.additional.96=-Dlog.dir.name=../xstoredata/xstore/log/xstore_services
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\xstore_services-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore REST Services once installed as a Windows service
    echo     %0 restart    restart Xstore REST Services once installed as a Windows service
    echo     %0 stop       stop Xstore REST Services once installed as a Windows service
    echo     %0 install    install Xstore REST Services as a Windows service
    echo     %0 remove     uninstall Xstore REST Services as a Windows service
...
:console
SET my_title=Xstore REST Services Console (%RANDOM%)
