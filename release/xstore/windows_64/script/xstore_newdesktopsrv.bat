...
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\xstore_newdesktopsrv.conf"
set _WRAPPED_APP=xstore_newdesktopsrv
set PLATFORM=windows_64
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=%ROOT_DIR%jre
set JAVA=%JAVA_HOME%\bin\xstore_newdesktopsrv.exe
set PIDDIR=%_REALPATH%tmp\
set OS_ADDITIONAL_PROP_1=wrapper.java.additional.96=-Dlog.dir.name=../xstoredata/xstore/log/xstore_newdesktopsrv
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\xstore_newdesktopsrv-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore New Desktop Server once installed as a Windows service
    echo     %0 restart    restart Xstore New Desktop Server once installed as a Windows service
    echo     %0 stop       stop Xstore New Desktop Server once installed as a Windows service
    echo     %0 install    install Xstore New Desktop Server as a Windows service
    echo     %0 remove     uninstall Xstore New Desktop Server as a Windows service
...
:console
SET my_title=Xstore New Desktop Server Console (%RANDOM%)
"%_WRAPPER_EXE%" -c %_WRAPPER_CONF% %OS_ADDITIONAL_PROP_1% wrapper.java.command=%JAVA% %PARAMS%
goto :eof

:start
"%_WRAPPER_EXE%" -t %_WRAPPER_CONF% %OS_ADDITIONAL_PROP_1% wrapper.java.command=%JAVA% %PARAMS%
goto :eof

:stop
"%_WRAPPER_EXE%" -p %_WRAPPER_CONF% %OS_ADDITIONAL_PROP_1% wrapper.java.command=%JAVA% %PARAMS%
goto :eof

:install
"%_WRAPPER_EXE%" -i %_WRAPPER_CONF% %OS_ADDITIONAL_PROP_1% wrapper.java.command=%JAVA% %PARAMS%
goto :eof

:remove
"%_WRAPPER_EXE%" -r %_WRAPPER_CONF% %OS_ADDITIONAL_PROP_1% wrapper.java.command=%JAVA% %PARAMS%
goto :eof
