...
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\hardware-service.conf"
set _WRAPPED_APP=hardware-service
set PLATFORM=windows_64
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=%ROOT_DIR%jre
set JAVA=%JAVA_HOME%\bin\hardware-service.exe
set PIDDIR=%_REALPATH%tmp\
set OS_ADDITIONAL_PROP_1=wrapper.java.additional.96=-Dlog.dir.name=../xstoredata/xstore/log/hardware-service
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\hardware-service-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Hardware Service once installed as a Windows service
    echo     %0 restart    restart Xstore Hardware Service once installed as a Windows service
    echo     %0 stop       stop Xstore Hardware Service once installed as a Windows service
    echo     %0 install    install Xstore Hardware Service as a Windows service
    echo     %0 remove     uninstall Xstore Hardware Service as a Windows service
...
:console
SET my_title=Xstore Hardware Service Console (%RANDOM%)
