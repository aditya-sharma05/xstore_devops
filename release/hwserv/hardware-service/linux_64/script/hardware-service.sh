...
# Application
APP_NAME="hardware-service"
APP_LONG_NAME="Xstore Hardware Service"
...
# description: Xstore Hardware Service
...
WRAPPER_CONF=$REALPATH/wrapper/conf/hardware-service.conf
APPLICATION_DIRECTORY="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export PLATFORM=linux_64
export _WRAPPED_APP=hardware-service
export JAVA_HOME=$(cd "${APPLICATION_DIRECTORY}/../jre" && pwd)
export JAVA=$JAVA_HOME/bin/hardware-service
PLATFORMPATH=$REALPATH/$PLATFORM
OS_ADDITIONAL_PROP_1=wrapper.java.additional.96=-Dlog.dir.name=../xstoredata/xstore/log/hardware-service
...
WRAPPER_CMD=$PLATFORMPATH/bin/hardware-service-wrapper
